/* -----------------------
  INSTRUCCIONES:
  - Asegúrate de ejecutar tu server Node (server.js) en http://localhost:5000
  - Coloca este archivo en ./static/index.html o sírvase desde el mismo origen.
  - No abras el archivo con file:// (origin null) — usa http://
-------------------------*/

/* ---------- Estado ---------- */
let asteroids = [];           // lista breve recibida por /api/neos
let selectedAsteroid = null;  // objeto cargado por /api/neo/:id
let currentResults = null;
let selectedStrategyIndex = 0;
let animationFrame = null;
let stars = [];

/* ---------- Material properties (para ablación/haz de iones) ---------- */
...
async function loadNeos(query='') {
  ...
  select.innerHTML = '';
  if (!query) {
    const opt = document.createElement('option'); opt.text = 'Introduce término y pulsa Buscar'; opt.disabled = true;
    select.appendChild(opt);
    select.disabled = false;
    return;
  }
  try {
    const resp = await fetch(`/api/neos?query=${encodeURIComponent(query)}`);
    ...
    console.error('Error loadNeos:', err);
    alert('Error al consultar el servidor. Revisa consola.');
    const opt = document.createElement('option'); opt.text = 'Error de conexión'; opt.disabled = true; select.appendChild(opt);
    select.disabled = false;
  }
}
...
async function fetchNeoDetailAndSelect(id) {
  try {
    const resp = await fetch(`/api/neo/${encodeURIComponent(id)}`);
    ...
    const asteroid = {
      ...
      diameter: detail.estimated_diameter_m || (detail.estimated_diameter || {}).meters ? (detail.estimated_diameter_m || ((detail.estimated_diameter || {}).meters ? (((detail.estimated_diameter || {}).meters.estimated_diameter_min || 0) + ((detail.estimated_diameter || {}).meters.estimated_diameter_max || 0))/2 : 0)) : 0,
      velocity: (detail.approach && detail.approach.velocity_km_s ? detail.approach.velocity_km_s * 1000 : (detail.close_approach_data && detail.close_approach_data[0] && detail.close_approach_data[0].relative_velocity ? parseFloat(detail.close_approach_data[0].relative_velocity.kilometers_per_second)*1000 : 0)),
      mass: detail.mass_kg || (detail.analysis && detail.analysis.mass_kg) || 0,
      density: detail.density_kg_m3 || 2000,
      material: detail.material || (detail.analysis && detail.analysis.composition) || 'desconocido',
    ...
    detailsEl.innerHTML = `<strong>${asteroid.name}</strong>
      ...
      ${asteroid.analysis ? `<pre>${JSON.stringify(asteroid.analysis, null, 2)}</pre>` : ''}`;
  } catch (err) {
    console.error('fetchNeoDetailAndSelect error:', err);
    alert('No se pudo obtener detalle del asteroide. Revisa la consola.');
  }
}
...
/* ---------- Selección visual y preparación de semilla para ruta inmutable ---------- */
function selectAsteroid(asteroid, cardElement) {
  selectedAsteroid = asteroid;
  // seed determinista para la visualización (para que la trayectoria original no cambie entre estrategias)
  ...
  // show minimal info if not already rendered
  ...
  // draw baseline static for selected asteroid
...
function calculateKineticImpact(asteroid, timeSeconds) {
  const impactorMass = 500; // kg (ejemplo)
  const impactorVelocity = 10000; // m/s
  const beta = 1.9;
...
function calculateAllStrategies() {
  ...
  // pick best (max earthRadii among successful)
...
/* ---------- Animación de comparación (usa parámetros calculados) ---------- */
function calcMissDistance(deltaV, timeSeconds) { return deltaV * timeSeconds; }

function drawStaticForAsteroid() {
  // dibuja una representación simple (tierra + trayectoria original) sin animación
  const rect = canvas.getBoundingClientRect();
  ctx.clearRect(0,0,rect.width,rect.height);
  drawStarfield();
  // Tierra
...
function animateComparison(result) {
  ...
  const origEndX = centerX - earthR*0.6;
  const origEndY = centerY;
  const defEndX = origEndX + devPx;
  const defEndY = origEndY;
  const ctrlXorig = (startX + origEndX)/2;
  const ctrlY = startY - rect.height*0.14;
  ...
  function loop(now) {
    ...
    // deflected
    ctx.strokeStyle = result.success ? 'rgba(0,255,136,0.95)' : 'rgba(255,107,53,0.95)';
    ctx.lineWidth = 3;
